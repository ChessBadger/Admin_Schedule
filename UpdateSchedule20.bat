...
echo "Changing to repository directory: %REPO_DIR%" >> %LOGFILE%
cd /d %REPO_DIR%
IF %ERRORLEVEL% NEQ 0 (
    echo "Error changing to repository directory" >> %LOGFILE%
    exit /b %ERRORLEVEL%
)
echo "Changed to repository directory: %REPO_DIR%" >> %LOGFILE%
